...
from structure.models import TelAfricSubscribers, Payment, CallLog,User
...
def check_credit():
    ...
    if rate > 0:
        max_duration = int(balance / (rate / 60))
    else:
        max_duration = 0
    ...
    if response.status_code == 200:
        ...
        if transaction_data['status'] == 'success':
            ...
            if subscriber:
                ...
                return render_template('telafric/paymentconfirmation.html',form=form,amount=amount_in_main_currency)
            else:
                print("Subscriber not found")
                return jsonify({"error": "Subscriber not found"}), 404
        else:
